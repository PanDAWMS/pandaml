...
class PredictionUtils:
    ...
    def get_prediction(self, model_manager: ModelManager, r: pd.DataFrame, task_id: int) -> pd.DataFrame:
        ...
        if not DataValidator.validate_prediction(base_df, "RAMCOUNT", DataValidator.acceptable_ranges, jeditaskid, self.logger):
        ...
        try:
            ...
            if not DataValidator.validate_ctime_prediction(base_df, jeditaskid, DataValidator.additional_ctime_ranges, self.logger):
        ...
        try:
            ...
            if not DataValidator.validate_prediction(base_df, "CPU_EFF", DataValidator.acceptable_ranges, jeditaskid, self.logger):
